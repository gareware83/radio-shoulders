...
### 3. The Vivado script does not currently build

`create_project.tcl` was changed to disable Scatter-Gather, and the run has not succeeded since. Sequence of failures and fixes, most recent last:

1. `[Ip 78-92] Failed to extract configurable options` / `No valid slave interface could be found to connect to M_AXI_MM2S`. Two hypotheses tried and **both wrong**: restoring `c_sg_include_stscntrl_strm {0}` did not help, and the IP config is demonstrably fine because the GP1 → `S_AXI_LITE` automation on the preceding line *succeeds*.
...
3. `[BD 5-313] Found unsupported IP 'axi_interconnect:1.7'` — `get_ipdefs -all` returns every version and `[lindex ... 0]` picked the deprecated one. Added `latest_ipdef` to select the highest version by dictionary sort. **This fix is untested.**

**If it still fails, bisect:** set `c_include_sg` back to `{1}`, change nothing else. Passing confirms Simple mode is the trigger; failing identically means the cause is outside the DMA config entirely and the last three changes were chasing the wrong thing.

Note the other four IP lookups still use `[lindex ... 0]`. They work because those IPs are single-version in this catalog, but the pattern is unsafe and worth cleaning up once the build is green.
