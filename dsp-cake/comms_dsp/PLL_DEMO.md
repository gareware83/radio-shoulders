...
- **Stimulus**: `test_bench/sin_cos_lut_gen.py --stimulus` generates
  `cos.dat`/`sin.dat` — a complex tone `A·e^{j(2π·f·n/fs + φ₀)}`, written as
...
- **Run it**: standard Vivado behavioral simulation, `tb_pll` as the
  ...
  wherever the simulator's working directory ends up (typically
  `<vivado-project>.sim/sim_1/behav/xsim/`) — Vivado does not auto-copy
  plain data files the way it does IP memory-init files, so this is a
  manual copy after regenerating the stimulus.
...
2. **Did the derotated signal land on a stable point?** `min(|I|,|Q|) /
   ...
   would mean it collapsed onto one axis instead, locking onto the wrong, unstable equilibrium. The parent project runs the full RX chain on a Zybo dev
