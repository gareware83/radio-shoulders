...
def make_img():
    ...
    # Two configs per .dtb: a ramdisk-root one (default, kept first so
    # existing bare `bootm $load_addr` calls are unaffected) and a
    # ramdisk-less one, so bootargs' root= can point at a real
    # persistent partition instead of the ramdisk.
    dtb_args = [a for a in sys.argv[2:] if a.endswith('.dtb')]
    other_args = [a for a in sys.argv[2:] if not a.endswith('.dtb')]
    if not dtb_args:
        raise SystemExit('mkfit.py: at least one .dtb file is required')
    fpga = Fpga(other_args[0]) if other_args else None

    configs = []
    for dtb in dtb_args:
        board = os.path.splitext(os.path.basename(dtb))[0]
        fdt = Fdt(dtb)
        configs.append(Configuration(board + ' (ramdisk root)', kernel,
                                      ramdisk, fdt, fpga,
                                      name='conf-' + board + '-ramdisk'))
        configs.append(Configuration(board + ' (persistent root)', kernel,
                                      None, fdt, fpga,
                                      name='conf-' + board + '-rootfs'))
    tree = ImageTree('Zybo/PYNQ custom configuration', configs)
